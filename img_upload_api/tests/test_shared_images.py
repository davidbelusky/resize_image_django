...
class SharedImagesTest(APITestCase):
    def setUp(self):
        ...
        for count,user in enumerate(self.user_list,1):
            ...
            if count == 1: shared_user_list = [self.user2.id,self.user3.id]
            if count == 2: shared_user_list = [self.user1.id]
            if count == 3: shared_user_list = ''
            img_object = Images.objects.create(img_name=f'test{str(count)}',owner=user,uploaded_image=image_path)
            img_object.save()
        ...
        for count,user in enumerate(self.user_list,1):
            ...
            if count == 2: shared_user_list = [self.user1.id]
            if count == 3: shared_user_list = ''
            img_object = StyleImage.objects.create(img_name=f'test_style{str(count)}',owner=user,styled_image=image_path,original_image=original_image)
            img_object.save()
            for share_user_add in shared_user_list:
                img_object.share_user.add(share_user_add)
                img_object.save()
    ...
    def test_share_users(self):
        ...
        models_dict = {'Images':{'url':self.url_images,'dict_key':'images'},'StyleImages':{'url':self.url_imagestyle,'dict_key':'styled_images'}}
        for model,values in models_dict.items():
            for user in self.user_list:
                ...
                if user.username == 'user1':
                    self.assertEqual('user2',response.data[values['dict_key']][0]['owner'])
                    self.assertIn(user.id, response.data[values['dict_key']][0]['share_user'])
                if user.username == 'user2':
                    self.assertEqual('user1',response.data[values['dict_key']][0]['owner'])
                    self.assertIn(user.id, response.data[values['dict_key']][0]['share_user'])
