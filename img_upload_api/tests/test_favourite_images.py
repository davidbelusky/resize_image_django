...
class Test_show_favourite_images(APITestCase):
    def setUp(self):
        ...
        for count in range(1,4):
            ...
            if count == 2: favourite = True
            else: favourite = False
            if count == 3: user=self.user2
            else: user=self.user1
            Images.objects.create(img_name=f'test{str(count)}',owner=user,uploaded_image=image_path,favourite=favourite)
        ...
        for count in range(1, 4):
            ...
            StyleImage.objects.create(img_name=f'test_style{str(count)}', owner=user, styled_image=image_path,favourite=favourite,original_image=original_image)
